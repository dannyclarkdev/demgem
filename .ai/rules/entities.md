...
## The family tree is one query through the relations scope and a walk in memory
entity_relations.kinship (Kinship: parent, child, sibling, spouse) is read from the source's side, and Kinship::reverse() is the target's word. Relations::family() loads every kinship row the viewer may see in the campaign through EntityRelation::visibleTo(), both ends eager-loaded, and walks two generations up and down in memory. That is the whole leak design: a row the scope refused is not in the graph, so a GM-only grandmother is never reached through a revealed mother. Do not walk hop by hop with per-hop queries, and do not infer siblings from shared parents. The "hidden" mark is for a GM and means the party would not see the row or the relative. The labels fill in from the kinship when blank and a GM's own words win; a plain relationship still requires a label (required_without:kinship).
