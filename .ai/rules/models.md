...
## A date in the world is three columns read as one GameDate, and the calendar's lists are JSON
Every in-game date is three integer columns (year, month, day) read through GameDateCast with a column prefix: `'in_game_start' => GameDateCast::class.':in_game_start'` maps to in_game_start_year/month/day; the entity's cast prefix is `happens`, so the columns are happens_year/month/day. Never store a day number: it only means something under one shape of the calendar, and the GM edits the shape. The triple sorts in SQL on its own and keeps its meaning when a month is renamed or given a day. The three are nullable as a group; the cast writes all or none.

`calendars.months`, `weekdays` and `moons` are JSON lists on one row, a recorded exception to "a list gets a child table": they are one configuration read whole, replaced whole on save, never queried by row, never gated. `Calendar::reckoning()` is the one place the row is read into `Reckoning`, and it casts each value because JSON returns 8.0 as 8.

## A decision is gated the clock's way, and an arc is an entity with two nullable columns pointing at it
`decisions` is a GM-written table with `player_visible` and `Decision::scopeVisibleTo()` on that column alone. The session link is loaded separately through `GameSession::visibleTo()`, keyed by id, and handed to the Blade: a revealed decision from a GM-only session keeps its words and loses the link. Both halves in queries, never an `@if`. Not a mention source, like a secret: `[[links]]` render but a rename does not rewrite them.
...
## The screen is two columns on the campaign, resolved through the strictest gate
campaigns.screen_focus (ScreenFocus) and campaigns.screen_entity_id are scalars on the campaign, not a table: one screen, one thing on it. Campaign::screen() is the one reader: it loads the page through Entity::visibleToParty() (Players only, no Selected, no own-character) and resolves a focus that needs a page and gets none back to null. So a handout the GM took back comes off the wall on the next render with the columns left alone; nothing else writes them, and SetScreen is the one writer. DeleteEntity clears both columns itself, because an entity soft-deletes and nullOnDelete never fires. The export carries both keys and the importer remaps the id after the entities, like active_combatant_id after the combatants.
